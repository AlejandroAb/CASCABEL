...
vsearchVersion = "**TBD**"
vsearchV = subprocess.run(['vsearch', '--version'], stdout=subprocess.PIPE)
...
if snakemake.config["chimera"]["search"] == "T":
    ...
    quimeraStr+=":green:`- Sequences target as non chimeras:` "+snakemake.wildcards.PROJECT+"/runs/"+snakemake.wildcards.run+"/"+snakemake.wildcards.sample+"_data/chimera/non_chimeras.txt\n\n"
...
if snakemake.config["demultiplexing"]["demultiplex"] == "T":
    ...
    splitStr+="----------------\n"
    splitStr+="Library splitting, also known as demultiplexing is carried on several steps.\n\n"
...
trimmedStr =  ":red:`Total number of reads after trimming:` "+str(trimmedCounts)+ "\n\n"
trimmedStr += ":red:`Percentage of reads vs raw reads:` "+str(prcTrimmedRaw)+"%\n\n"
trimmedStr+=":red:`Percentage of reads after demultiplexed reads vs :` " + str(prcTrimmedSplit) + "%\n\n"
if cutSequences:
...
:red:`Number of total reads:` {rawCounts}
...
:red:`Number of peared reads:` {pearedCounts} =  {prcPeared}%
...
:red:`Total number of accepted reads:` {totalAssigned} = {prcPearedAssigned}% of the peared reads or {prcRawAssigned}% of the raw reads
...
**To see a better detail on the library sample distribution please refere to the file:** {snakemake.wildcards.PROJECT}/runs/{snakemake.wildcards.run}/{snakemake.wildcards.sample}_data/seqs_fw_rev_filtered.dist.txt
...
.. image:: report_files/seqs_fw_rev_filtered.{snakemake.wildcards.sample}.dist.png

The previous chart shows the number of clean reads per sample. The bars are sorted from left to right, according to the metadata input file.
...
The taxonomic report performed in combination with the different libraries supplied, can be found at the following link: taxo_report_ ({snakemake.wildcards.PROJECT}/runs/{snakemake.wildcards.run}/report_all.html)

    .. _taxo_report: otu_report_{snakemake.config[assignTaxonomy][tool]}.html
